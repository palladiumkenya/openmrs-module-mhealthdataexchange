package org.openmrs.module.mhealthdataexchange.mhealthDataexchangeUtils;
...
public class Utils {
	...
	public static List<Obs> getNLastObs(Concept concept, Patient patient, Integer nLast) throws Exception {
		List<Obs> obs = Context.getObsService().getObservations(
		    Arrays.asList(Context.getPersonService().getPerson(patient.getPersonId())), null, Arrays.asList(concept), null,
		    null, null, null, nLast, null, null, null, false);
		return obs;
	}
	...
	/**
	 * getEncounters(Patient who, Location loc, Date fromDate, Date toDate, Collection<Form>
	 * enteredViaForms, Collection<EncounterType> encounterTypes, Collection<Provider> providers,
	...
	public static List<Obs> getEncounterObservationsForQuestions(Person patient, Encounter encounter, List<Concept> questions) {
		 ...
		 */
		return Context.getObsService().getObservations(Arrays.asList(patient), Arrays.asList(encounter), questions, null,
		    null, null, null, null, null, null, null, false);
	}
	...
	public static Location getLocationFromMFLCode(String mflCode) {
		...
		try {
			Context.addProxyPrivilege(PrivilegeConstants.GET_LOCATIONS);
			Context.addProxyPrivilege(PrivilegeConstants.GET_GLOBAL_PROPERTIES);
			LocationAttributeType facilityMflCode = Context.getLocationService().getLocationAttributeTypeByUuid(
			    MASTER_FACILITY_CODE);
			...
			List<Location> locationForMfl = Context.getLocationService().getLocations(null, null, mflCodeMap, false, null,
			    null);
