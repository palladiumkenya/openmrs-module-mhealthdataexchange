...
import org.openmrs.module.mhealthdataexchange.models.observation.OBSERVATION_RESULT;
 ...
 */
public class MhealthdataexchangePerson {
	...
	public static MhealthdataexchangePerson fill(JsonObject jsonobj) throws IOException {
		ObjectMapper objectMapper = new ObjectMapper();
		MhealthdataexchangePerson entity = new MhealthdataexchangePerson();
		if (jsonobj.containsKey("PATIENT_VISIT")) {
			PATIENT_VISIT patient_visit = objectMapper.readValue(jsonobj.getJsonObject("PATIENT_VISIT").toString(),
			    PATIENT_VISIT.class);
			entity.setPatient_visit(patient_visit);
		}
		
		if (jsonobj.containsKey("MESSAGE_HEADER")) {
			MESSAGE_HEADER message_header = objectMapper.readValue(jsonobj.getJsonObject("MESSAGE_HEADER").toString(),
			    MESSAGE_HEADER.class);
			entity.setMessage_header(message_header);
		}
		if (jsonobj.containsKey("PATIENT_IDENTIFICATION")) {
			PATIENT_IDENTIFICATION patient_identification = objectMapper.readValue(
			    jsonobj.getJsonObject("PATIENT_IDENTIFICATION").toString(), PATIENT_IDENTIFICATION.class);
	...
	public static List<MhealthdataexchangePerson> fillList(JsonArray jsonarray) throws IOException {
		if (jsonarray == null || jsonarray.size() == 0)
			return null;
		List<MhealthdataexchangePerson> olist = new ArrayList<MhealthdataexchangePerson>();
