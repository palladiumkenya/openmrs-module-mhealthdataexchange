...
public class PATIENT_IDENTIFICATION {
	...
	public static PATIENT_IDENTIFICATION fill(JsonObject jsonobj) throws IOException {
		...
		if (jsonobj.containsKey("EXTERNAL_PATIENT_ID")) {
			EXTERNAL_PATIENT_ID external_patient_id = objectMapper.readValue(jsonobj.getJsonObject("EXTERNAL_PATIENT_ID")
			        .toString(), EXTERNAL_PATIENT_ID.class);
		...
		if (jsonobj.containsKey("PATIENT_NAME")) {
			PATIENT_NAME patient_name = objectMapper.readValue(jsonobj.getJsonObject("PATIENT_NAME").toString(),
			    PATIENT_NAME.class);
			entity.setPatient_name(patient_name);
		}
		if (jsonobj.containsKey("MOTHER_NAME")) {
			MOTHER_NAME mother_name = objectMapper.readValue(jsonobj.getJsonObject("MOTHER_NAME").toString(),
			    MOTHER_NAME.class);
		...
		if (jsonobj.containsKey("PATIENT_ADDRESS")) {
			PATIENT_ADDRESS patient_address = objectMapper.readValue(jsonobj.getJsonObject("PATIENT_ADDRESS").toString(),
			    PATIENT_ADDRESS.class);
	...
	public String toString() {
		return "PATIENT_IDENTIFICATION{" + "external_patient_id=" + external_patient_id + ", internal_patient_id="
		        + internal_patient_id + ", patient_name=" + patient_name + ", mother_name='" + mother_name + '\''
		        + ", date_of_birth='" + date_of_birth + '\'' + ", sex='" + sex + '\'' + ", patient_address="
		        + patient_address + ", phone_number='" + phone_number + '\'' + ", marital_status='" + marital_status + '\''
		        + ", death_date='" + death_date + '\'' + ", death_indicator='" + death_indicator + '\'' + '}';
	}
}
